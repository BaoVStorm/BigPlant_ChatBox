...
const REPO_URL = "https://huggingface.co/vilm/vinallama-7b-chat-GGUF/resolve/main";
const MODEL_FILES = ["vinallama-7b-chat_q5_0.gguf"];

const outputArg = process.argv[2] || "./models";
const outputDir = resolve(outputArg.endsWith(".gguf") ? dirname(outputArg) : outputArg);

try {
  console.log("Downloading VinaLLaMA-7B-Chat GGUF Q5_0");
  ...
  console.log("\nModel downloaded.");
  console.log("Set LLM_MODEL_PATH=./models/vinallama-7b-chat_q5_0.gguf");
  console.log("Set LLM_PROMPT_FORMAT=chatml or keep LLM_PROMPT_FORMAT=auto");
} catch (error) {
  console.error(`\n${error.message}`);
  process.exit(1);
}
